...
CREATE TABLE perfumes (
    ...
    brand_name VARCHAR(100), -- Keep for backward compatibility
    gender VARCHAR(20) CHECK (gender IN ('Men', 'Women', 'Mixte')),
    description TEXT,
    price DECIMAL(10,2),
    image_url TEXT,
...
-- Create policies for authenticated users to insert/update (optional for admin features)
...
CREATE POLICY "Enable insert for authenticated users only" ON perfumes FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Enable update for authenticated users only" ON perfumes FOR UPDATE USING (auth.role() = 'authenticated');
